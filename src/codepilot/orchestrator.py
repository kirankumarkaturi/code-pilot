...
class Orchestrator:
    ...
    def run_once(self, hitl_approver=None) -> dict:
        """
        Full pipeline: fetch → classify → explore → implement → test → HITL → git commit → PR.
        ...
        """
        issues = self.github.fetch_open_issues()
        if not issues:
        ...
        task.state = TaskState.PR_OPENED if pr_url else (TaskState.DONE if pr_res.ok else TaskState.FAILED)
