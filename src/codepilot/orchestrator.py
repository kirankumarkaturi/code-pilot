...
class Orchestrator:
    ...
    def _build_commit_message(self, issue: Issue, promoted_files: list[str], fix_source: str) -> str:
        files_text = ", ".join(promoted_files) if promoted_files else "none"
        return (
            f"fix(#{issue.issue_id}): {issue.title}\n\n"
            f"- Updated files: {files_text}\n"
            f"- Generated by CodePilot using {fix_source or 'unknown'} flow\n"
            f"- Closes #{issue.issue_id}"
        )
            ...
            if hitl_needed and promoted_files and pr_res.ok:
                ...
                if hitl_approved:
                    ...
                    commit_msg = self._build_commit_message(
                        ...
                        fix_source=code_res.payload.get("fix_source", "unknown"),
                    )
